...
module.exports = ( router ) => {
  ...
  router.post('/newBlog', (req, res) => {
    ...
    if( !req.body.title ) {
      res.json({ success: false, message: 'Must provide an title' });
    } else {
      if( !req.body.body ) {
        res.json({ success: false, message: 'Must provide a body' });
      } else {
        if( !req.body.createdBy ) {
          res.json({ success: false, message: 'Must provide a createdBy' });
        } else {
            ...
            if( err ) {
              if( err.errors ){
                if( err.errors.title ){
                  ...
                  res.json({ success:false, message: 'Could not save to database. Error: ', err });
                }
              }
          } else {
        ...
        if(!blog){
          ...
          User.findOne({ _id: req.decoded.userId }, (err, user) => {
            if(err){
              ...
              if(!user){
                res.json({ success: false, message: 'Unabe to authenticate user' });
              } else {
                  ...
                  if( user.username !== blog.createdBy ){
                    ...
                    blog.save( (err) => {
                      if(err){
                        ...
                        res.json({ success: true, message: 'Blog has been updated' });
                      }
                    })
                  }
                }
                  ...
                  if( user.username !== blog.createdBy ){
                    ...
                    blog.remove( (err) => {
                      if(err){
                        ...
                        res.json({ success: true, message: 'Blog has been removed' });
                      }
                    })
                  }
                }
  ...
  router.put('/likeBlog', (req, res) => {
    if(!req.body.id){
      ...
      Blog.findOne({ _id: req.body.id }, (err, blog) => {
        if(err){
          res.json({ success: false, message: 'An erro has ocurred' });
        } else {
          if(!blog){
