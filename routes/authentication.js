...
module.exports = ( router ) => {
  ...
  router.get('/verifyUsername/:username', ( req, res ) => {
          ...
          if( err ){
            ...
            if( !user ){
              ...
              if( !passwordValidation ){
                ...
                const token = jwt.sign({ userId: user._id }, config.specialpassword, { expiresIn: '24h' });
  ...
  router.use( ( req, res, next ) => {
    ...
    if( !token ){
      ...
      jwt.verify( token, config.specialpassword, ( err, decoded ) => {
        if( err ){
